...
`;
    ...
    if (error.constructor && error.constructor.name === 'GoogleGenerativeAIResponseError') {
        ...
        if (safetyRatings.length > 0) {
            message += ` Blocked due to: ${safetyRatings.map(r => r.category).join(', ')}.`;
        }
        return NextResponse.json({ error: message, details: safetyRatings }, { status: 400 });
    }
    return NextResponse.json({ error: "An unexpected error occurred while processing the chat.", details: error.message }, { status: 500 });
  }
}
