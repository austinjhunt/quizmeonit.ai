...
export default function ExplanationChat({ question }: ExplanationChatProps) {
  ...
  useEffect(() => {
    ...
    if (messages.length === 0) {
      setMessages([
        { sender: 'ai', text: "Hello! How can I help you understand this explanation better?" }
      ]);
    }
  ...
  const handleSendMessage = async (event: FormEvent<HTMLFormElement>) => {
    ...
    try {
      ...
      if (!response.ok) {
        ...
        setMessages(prevMessages => [...prevMessages, { sender: 'ai', text: `Sorry, I encountered an error: ${errorMsg}` }]);
      } else {
      ...
      setChatError(errorMsg);
      setMessages(prevMessages => [...prevMessages, { sender: 'ai', text: `Sorry, an unexpected error occurred: ${errorMsg}` }]);
    } finally {
      setIsLoading(false);
    }
  };
  ...
  return (
    <div className="mt-6 p-5 bg-slate-800/80 border border-slate-700/80 rounded-xl shadow-lg backdrop-blur-md">
      ...
      {chatError && (
        <div className="mb-3 bg-red-600/20 border border-red-500/50 text-red-300 px-3.5 py-2.5 rounded-lg text-xs shadow" role="alert">
